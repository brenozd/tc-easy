...
_add_route() {
    ...
    if _get_dev_qdisc "$__add_route_dev" "root" && [ "$__g_dev_qdisc" != "prio" ]; then
        ...
        tc qdisc add dev "$__add_route_dev" root handle 1: prio
        tc qdisc add dev "$__add_route_dev" parent 1:1 handle 10: sfq
        tc qdisc add dev "$__add_route_dev" parent 1:2 handle 20: htb
        tc qdisc add dev "$__add_route_dev" parent 1:3 handle 30: sfq
        ...
        tc class add dev "$__add_route_dev" parent 20: classid 20:1 htb rate "$__add_route_dev_speed"mbit ceil "$__add_route_dev_speed"mbit
    fi
    ...
    tc class add dev "$__add_route_dev" parent 20:1 classid 20:"$__add_route_new_handle" htb rate "$__add_route_bandwidth"mbit ceil "$__add_route_bandwidth"mbit
    ...
    if [ -n "$__add_route_netem_params" ]; then
        ...
        tc qdisc add dev "$__add_route_dev" parent 20:"$__add_route_new_handle" handle "$__add_route_new_handle":1 netem $__add_route_netem_params
    fi

    tc filter add dev "$__add_route_dev" protocol ip parent 20: u32 match ip src "$__add_route_src_ip" match ip dst "$__add_route_dst_ip" flowid 20:"$__add_route_new_handle"
}
